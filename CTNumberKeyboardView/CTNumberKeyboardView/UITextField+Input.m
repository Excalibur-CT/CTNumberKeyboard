...
- (void)inputInsetBlankText:(NSString *)text limit:(NSInteger)limit
{
    ...
    if (![text isEqualToString:@""])
    {
        NSString * whiStr = [newText stringByReplacingOccurrencesOfString:@" " withString:@""];
        NSMutableString * result = [[NSMutableString alloc] initWithString:whiStr];
        NSInteger count = result.length/4;
        ...
        for (int i = 0; i < count; i++) {
            [result insertString:@" " atIndex:i*5+4];
        }
        self.text = result;
    }
    else
